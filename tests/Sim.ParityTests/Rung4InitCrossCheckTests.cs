...
// vType defaults resolver init cross-check for rung 4 (two-vehicle following, CLAUDE.md rule 6:
// match vType/init first when a parity gap appears). Unlike rung 1's golden.vtype.json (a single
// top-level "vType" object), rung 4's golden.vtype.json is keyed by vType id under "vTypes"
// (leader, follower) -- this exercises the generalized override resolver (Sim.Ingest.
// VTypeDefaults.ResolvePassenger applying rou.xml overrides on top of vClass defaults) for BOTH
// the leader's maxSpeed="5.00" override and both vTypes' sigma="0" override, run BEFORE the
// trajectory parity test below.
public class Rung4InitCrossCheckTests
{
    private static readonly string ScenarioDir = Path.Combine(RepoRoot(), "scenarios", "02-two-vehicle-following");

    [Fact]
    public void ResolvePassenger_Leader_MatchesGoldenVTypeJson()
    {
        AssertResolvesTo("leader");
    }

    [Fact]
    public void ResolvePassenger_Follower_MatchesGoldenVTypeJson()
    {
        AssertResolvesTo("follower");
    }

    private static void AssertResolvesTo(string vTypeId)
    {
        var demand = DemandParser.Parse(Path.Combine(ScenarioDir, "rou.rou.xml"));
        var rawVType = demand.VTypesById[vTypeId];
        var resolved = VTypeDefaults.ResolvePassenger(rawVType);

        var golden = LoadGoldenVType(vTypeId);
