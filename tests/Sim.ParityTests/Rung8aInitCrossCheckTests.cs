...
// vType defaults resolver init cross-check for rung 8a (platoon shockwave). golden.vtype.json is
// keyed by vType id under "vTypes" with a single entry, "passenger0" -- run BEFORE the
// trajectory parity test, exactly like Rung6InitCrossCheckTests.
public class Rung8aInitCrossCheckTests
{
    private static readonly string ScenarioDir = Path.Combine(RepoRoot(), "scenarios", "06-two-lane-cruise");
