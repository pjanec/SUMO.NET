...
- [~] **T2.4 (experimental)** Spatial look-ahead (owner: v49/v229 turn in off the connecting-lane centerline, then compensate): aim the front at a point `LookAheadMeters` ahead ON the upcoming lane centerline (free from PoseResolver — advance the front arc position, chord to it = predictor direction; the DDS forward-path analog). At LA=3 offset-from-centerline v49 1.15→0.49 m, v229 1.52→0.33 m, fleet mean reversals 0.48→0.32. BUT the resolved point is unstable across some junctions — ~12 vehicles go 0-1→11-12 reversals, lat-accel max 70→367. Shipped OFF by default (`LookAheadMeters=0`=v3; `IGBRIDGE_LOOKAHEAD=<m>` enables). Needs look-ahead-point stabilization before default-on. Motion 11/11, IgBridge 11/11, parity 654/4 byte-identical. See DECISIONS §5.13.
