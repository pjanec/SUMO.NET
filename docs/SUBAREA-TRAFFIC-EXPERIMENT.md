...
- **Halo convergence** → `experiments/subarea/RESULTS-halo.md` (how small a halo reproduces full-macro
  ...
  **Result (verified independently):** for a fixed 1800 m inner box, the inner-box mean speed,
  vehicle-distance and distinct-vehicle count match the full-macro ("infinite halo") ground truth to
  **within 1 % already at h = 0** (no halo), and stay in that sub-1 % band out to a 2400 m halo with no
  trend → **h\* = 0** here. So the aggregate flow/volume in the box is set almost entirely by *which
  vehicles cross its boundary and when* — which `cutRoutes` reproduces at the boundary — not by the
  surrounding terrain. **Caveat:** this is a best case (homogeneous grid, spatially-uniform demand, mild
  congestion) and the metrics are aggregate; a real net with directional bottlenecks or congestion
  propagating in from outside would need a non-zero halo, and entry *microstructure* (platooning) isn't
  probed here. The rule-of-thumb halo (a few hundred m to ~1 km) stands as the safe default; the real
  number must be measured on the Geneva net. Scripts: `experiments/subarea/halo/`.
  **Cross-cutting gotcha found:** SUMO `vehroute-output` **without** `--vehroute-output.write-unfinished`
  silently drops vehicles still en route at sim end (here 648/4500 = 14.4 %). Any cropping/golden pipeline
  that cuts from vehroutes MUST pass `--vehroute-output.write-unfinished` (+ `--exit-times`), or every
  crop inherits an identical demand shortfall that masquerades as spatial non-convergence.
