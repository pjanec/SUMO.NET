...
## 9. The live city — settings, net, ambient flow (from `SceneGen.BuildLiveCity`)

Lift these into the host (all `src/Sim.Viz/SceneGen.cs`):
...
- **Key code:** `src/Sim.IgBridge/{IgBridgeRunner,IgBridgeSession,PedStream,IgSample}.cs`,
  ...
  `src/Sim.Viz/SceneGen.cs` (`BuildLiveCity`), `demos/City3D/CityLib/{Reconstructor,PedReconstructor}.cs`.
